...
Request const &								Response::getRequest(void) const
{
    return (m_request);
}

Route const &									Response::getRoute(void) const
{
    return (m_route);
}

int const &										Response::getStatusCode(void) const
{
    return (m_status_code);
}

std::string const &								Response::getStartLine(void) const
{
    return (m_start_line);
}

std::map<std::string, std::string> const &		Response::getHeadersMap(void) const
{
    return (m_headers_map);
}

std::string const &								Response::getHeadersStr(void) const
{
    return (m_headers_str);
}

std::string const &								Response::getBody(void) const
{
    return (m_body);
}
...
string_pair_t			Response::_buildDate(void)
{
    std::time_t		rawtime;
    struct std::tm	*ptm;
    char			buf[50];
...
// WIP
string_pair_t			Response::_buildAllow(void)
{
...
string_pair_t			Response::_buildServer(void)
{
    return (std::make_pair("Server", "Websurf/1.0.0 (Unix)"));  // TODO PROG_NAME
}

string_pair_t			Response::_buildConnection(void)
{
    return (std::make_pair("Connection", "close"));
}

string_pair_t			Response::_buildTransferEncoding(void)
{
    return (std::make_pair("Transfer-Encoding", "chunked"));
}

string_pair_t			Response::_buildContentLength(void)
{
    int			string_size;
...
string_pair_t			Response::_buildContentType(void)
{
...
void					Response::buildHeaders(void)
{
