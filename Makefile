...
SOURCE_DIR  = srcs
HEADER_DIR  = includes tests/unittests
TEST_DIR    = tests
LIB_BASE	= libs
...
HEADER_DIR	+= $(PWD)/$(LIB_BASE)/openssl/includes
HEADER_DIR	+= $(PWD)/$(LIB_BASE)/curl/includes
HEADER_DIR	+= $(PWD)/$(LIB_BASE)/googletest/googletest/include
...
$(TEST_DIR)/%.o: $(TEST_DIR)/%.cpp
	$(CXX) -std=c++11 -fsanitize=address -c $< -o $@ $(HEADERS)
...
tester: $(OBJECTS) $(TST_OBJECTS) $(LIB_BASE)/googletest/lib/libgtest.a $(LIB_BASE)/openssl/lib/libssl.a $(LIB_BASE)/curl/lib/libcurl.a
	$(CXX) $(OBJECTS) $(TST_OBJECTS) -o $@ $(CFLAGS) $(INCLUDES) $(LIBS) $(CLINKS) -lcurl -lgtest -lgtest_main
